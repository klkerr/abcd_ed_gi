...
abcd.ed.gi$b.p.gi_sum.centered = scale(abcd.ed.gi$b.p.gi_sum,center=TRUE,scale=FALSE)
abcd.ed.gi.m_crpbi$b.p.gi_sum.centered = scale(abcd.ed.gi.m_crpbi$b.p.gi_sum,center=TRUE,scale=FALSE)
abcd.ed.gi.f_crpbi$b.p.gi_sum.centered = scale(abcd.ed.gi.f_crpbi$b.p.gi_sum,center=TRUE,scale=FALSE)
...
cont.vars=c("b.bmi","y1.interview_age","b.gi_sum","y1.ed_sum")
...
writeLines("###############################")
writeLines("Whole sample correlation matrix")
writeLines("###############################")

data=abcd.base[c(cont.vars,"b.cbcl_scr_dsm5_anxdisord_t","b.crpbi_mother","b.crpbi_father","y1.interview_age")]
# data=data[complete.cases(data),]
cor(data,method="pearson",use="pairwise.complete.obs")

writeLines("###########")
writeLines("LME results")
writeLines("###########")
...
writeLines("Results for GI symptoms and paternal acceptance:")	
summary(y1.ed_pred_b.gi_b.fcrpbi)
r.squaredGLMM(y1.ed_pred_b.gi_b.fcrpbi)

writeLines("##############################")
writeLines("LME results by participant sex")
writeLines("##############################")
